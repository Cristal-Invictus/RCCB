...
function populateParoisses(v) {
  ...
  const list = VICARIATS_PAROISSES[v] || [];
  paroisseSelect.disabled = list.length === 0;
  list.forEach((p) => {
...
form.addEventListener('submit', async (e) => {
  ...
  if (!VICARIATS_PAROISSES[data.vicariat]?.includes(data.paroisse)) {
    flash.textContent = 'Veuillez choisir une paroisse correspondant au vicariat sélectionné.';
